...
echo "🔍 Verificando que los endpoints healthz del Controller Manager estén protegidos por RBAC"

# 1️⃣ Verificar la configuración de livenessProbe
LIVENESS_PROBE=$(oc -n openshift-kube-controller-manager get cm kube-controller-manager-pod -o json | jq -r '.data."pod.yaml"' | jq '.spec.containers[].livenessProbe')
...
READINESS_PROBE=$(oc -n openshift-kube-controller-manager get cm kube-controller-manager-pod -o json | jq -r '.data."pod.yaml"' | jq '.spec.containers[].readinessProbe')
...
# 3️⃣ Verificar los endpoints del Controller Manager
ENDPOINTS=$(oc -n openshift-kube-controller-manager describe endpoints)

if [ -z "$ENDPOINTS" ]; then
    echo "❌ Fallo: No se encuentran los endpoints del Controller Manager."
    exit 1
else
    echo "✅ Endpoints del Controller Manager encontrados."
fi
...
# Cambiar al namespace del Controller Manager
...
echo "✅ Éxito: Los endpoints healthz del Controller Manager están protegidos por RBAC correctamente."
exit 0
