...
echo "Checking permissions of static-pod-certs private key files"
...
do 
    ...
    # Buscar archivos .key en /etc/kubernetes/static-pod-certs/secrets/
    for file in $(oc exec -n openshift-kube-apiserver $i -c kube-apiserver -- \
        find /etc/kubernetes/static-pod-certs -type f -wholename '*/secrets/*.key')
    do
...
echo "Éxito en la verificación de permisos de los archivos de clave privada en static-pod-certs"
exit 0
